...
import logging
import time
from AI_agents.tools.retrosynthesis_planner.retro_star.common import prepare_starting_molecules, prepare_mlp, \
    prepare_molstar_planner, smiles_to_fp
from AI_agents.tools.retrosynthesis_planner.retro_star.model import ValueMLP
from AI_agents.tools.retrosynthesis_planner.retro_star.utils import setup_logger

import os
dirpath = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'retro_data')
...
class RSPlanner:
    def __init__(self,
                 ...
                 iterations=500,
                 use_value_fn=False,
                 starting_molecules=os.path.join(dirpath, 'dataset/origin_dict.csv'),
                 mlp_templates=os.path.join(dirpath, 'one_step_model/template_rules_1.dat'),
                 mlp_model_dump=os.path.join(dirpath, 'one_step_model/saved_rollout_state_1_2048.ckpt'),
                 save_folder=os.path.join(dirpath, 'saved_models'),
