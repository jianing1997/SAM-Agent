...
def segment_chemical_structures_from_file(
    ...
    segments = []
    for image in images:
        segs = segment_chemical_structures(image)
        for seg in segs:
            segments.append(seg)
    return segments
...
def segment_chemical_structures(
    ...
    segments = [(segments[i], image[bboxes[i][0]:bboxes[i][2],bboxes[i][1]:bboxes[i][3],:]) for i in range(len(segments))
                if segments[i].shape[0] > 0
                if segments[i].shape[1] > 0]
...
def save_images_pil(images: List[np.array], path: str, name: str) -> None:
    if not os.path.exists(path):
        os.makedirs(path, exist_ok=True)
    for index in range(len(images)):
        filename = f"{name}_{index}.png"
        file_path = os.path.join(path, filename)
        Image.fromarray(images[index]).save(file_path)
